...
void display_lpar_util()
{
 ...
 /*  double psd0,psd1,psd2,psd3,psd4,psd5; */
   ...
   perfstat_cpu_t *statp;
    ...
    perfstat_processor_pool_util_t poolstats;

    
    u_longlong_t xfers,wblks,rblks,rq,sq,hpi,rserv,wserv;
  ...
  fp2 = popen("/usr/sbin/lscfg -l vty0 | cut -d. -f 3 | cut -d- -f1", "r");
  if (fp2 == NULL) {
  ...
  while (fgets(path, sizeof(path)-1, fp2) != NULL) {
     if (debug==1) printf("lscfg output = %s \n",path);
  ...
  pclose(fp2);
    ...
    if (rc != 1) {
        perror("Error: rc code from : perfstat_partition_total");
        exit(-1);
    }
    perfstat_cpu_total(NULL,&cpustats,sizeof(perfstat_cpu_total_t),1);
    perfstat_processor_pool_util(NULL,&poolstats,sizeof(perfstat_processor_pool_util_t),1);
...
/* disk io */
        ...
        sq=diskstats.wq_depth;
        rserv=diskstats.time;
        wserv=diskstats.wq_time;
	/*        if (debug==1) printf("rserv= %lu \n",rserv);
	        if (debug==1) printf("wserv= %ld \n",wserv);
	        if (debug==1) printf("sq= %ld \n",sq);
	*/
...
/* check AIX versions */
    ...
    if ( fgets(str,256,fp) != NULL ) {
		...
		printf("AIX 5.3 must be at TL12 or above \n");
         ...
         if ( aixver == 6100 ) {
		if ( aixtl < 8 ) {
		printf("AIX 6.1 must be at TL8 or above \n");
    	        exit(0);
		}
			sprintf(vers,"%s","AIX616");
	 }
		...
		if ( aixtl < 2 ) {
			printf("AIX 7.1 must be at TL2 or above \n");
    	        	exit(0);
		}
			sprintf(vers,"%s","AIX616");
	 }
